...
listener tcp:Listener server2 = new (PORT2);
listener tcp:Listener server3 = new (PORT3);
listener tcp:Listener server4 = new (PORT4);
listener tcp:Listener server5 = new tcp:Listener(PORT5, {readTimeoutInMillis: 20000});

service "echoServer" on server1 {

    remote isolated function onConnect(tcp:Caller caller) {
        log:print("Join: " + caller.remotePort.toString());
    }

    remote isolated function onReadReady(tcp:Caller caller) {
    ...
    remote isolated function onError(tcp:Caller caller, error er) {
...
function process(any|error result, tcp:Caller caller) {
    if (result is [byte[], int]) {
        var [content, length] = result;
        if (length > 0) {
